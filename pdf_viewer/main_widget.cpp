...
void MainWidget::resizeEvent(QResizeEvent* resize_event) {
    ...
    if ((main_document_view->get_document() != nullptr) &&
        (main_document_view->get_zoom_level() == 0)) {
        main_document_view->fit_to_page_width();
...
void MainWidget::mouseMoveEvent(QMouseEvent* mouse_event) {
    ...
    if (is_rotated()) {
        // we don't handle mouse events while document is rotated becausae
        // proper handling would increase the code complexity too much to be
        // worth it
        return;
    }
    ...
    NormalizedWindowPos normal_mpos =
        main_document_view->window_to_normalized_window_pos(mpos);

    if (rect_select_mode) {
        if (rect_select_begin.has_value()) {
            AbsoluteDocumentPos abspos =
                main_document_view->window_to_absolute_document_pos(mpos);
    ...
    if (overview_resize_data) {
        // if we are resizing overview page, set the selected side of the
        // overview window to the mosue position float offset_diff_x = normal_x
        // - overview_resize_data.value().original_mouse_pos.first; float
        // offset_diff_y = normal_y -
    ...
    if (overview_move_data) {
        fvec2 offset_diff =
            fvec2(normal_mpos) -
            fvec2(overview_move_data.value().original_normal_mouse_pos);
        offset_diff[1] = -offset_diff[1];
        fvec2 new_offsets =
            overview_move_data.value().original_offsets + offset_diff;
    ...
    if (opengl_widget->is_window_point_in_overview(normal_mpos)) {
        link = doc()->get_link_in_pos(
            opengl_widget->window_pos_to_overview_pos(normal_mpos)
        );
        if (link) {
    ...
    if (is_dragging) {
        ...
        main_document_view->set_offsets(
            last_mouse_down_document_offset.x + diff_doc.x(),
    ...
    if (is_selecting) {
        ...
        int msecs_since_last_text_select =
            last_text_select_time.msecsTo(QTime::currentTime());
        if (msecs_since_last_text_select > 16 ||
            msecs_since_last_text_select < 0) {

            AbsoluteDocumentPos document_pos =
                main_document_view->window_to_absolute_document_pos(mpos);

            selection_begin = last_mouse_down;
            selection_end = document_pos;
            // fz_point selection_begin = { last_mouse_down.x(),
            // last_mouse_down.y()}; fz_point selection_end = { document_x,
            // document_y };

            main_document_view->get_text_selection(
                selection_begin, selection_end, is_word_selecting,
                main_document_view->selected_character_rects, selected_text
            );
...
MainWidget::MainWidget(
    ...
    main_document_view = new DocumentView(
        mupdf_context, db_manager, document_manager, config_manager, checksummer
    );
    opengl_widget = new PdfViewOpenGLWidget(
        main_document_view, pdf_renderer, config_manager, false, this
    );

    helper_document_view = new DocumentView(
        mupdf_context, db_manager, document_manager, config_manager, checksummer
    );
    helper_opengl_widget = new PdfViewOpenGLWidget(
        helper_document_view, pdf_renderer, config_manager, true
    );
    ...
    helper_opengl_widget->register_on_link_edit_listener(
        [this](OpenedBookState state) {
    ...
    text_command_line_edit_container_layout->addWidget(
        text_command_line_edit_label
    );
    ...
    text_command_line_edit_container->setLayout(
        text_command_line_edit_container_layout
    );
    ...
    on_command_done = [&](std::string command_name) {
        ...
        int page_number =
            QString::fromStdString(command_name).toInt(&is_numeric);
        if (is_numeric) {
            if (main_document_view) {
            ...
            std::unique_ptr<Command> command =
                this->command_manager->get_command_with_name(command_name);
            handle_command_types(std::move(command), 0);
        }
    };

    // when pdf renderer's background threads finish rendering a page or find a
    // new search result we need to update the ui
    QObject::connect(
        pdf_renderer, &PdfRenderer::render_advance, this,
        &MainWidget::invalidate_render
    );
    QObject::connect(
        pdf_renderer, &PdfRenderer::search_advance, this,
        &MainWidget::invalidate_ui
    );
    ...
    connect(validation_interval_timer, &QTimer::timeout, [&, INTERVAL_TIME]() {
        ...
        // detect if the document file has changed and if so, reload the
        // document
        if (main_document_view != nullptr) {
            Document* doc = nullptr;
            if ((doc = main_document_view->get_document()) != nullptr) {

                // Wait until a safe amount of time has passed since the last
                // time the file was updated on the filesystem this is because
                // LaTeX software frequently puts PDF files in an invalid state
                // while it is being made in multiple passes.

                if ((doc->get_milies_since_last_edit_time() <
                     (2 * INTERVAL_TIME)) &&
                    doc->get_milies_since_last_edit_time() > INTERVAL_TIME &&
                    doc->get_milies_since_last_document_update_time() >
                        doc->get_milies_since_last_edit_time()) {

                    // if (doc->get_milies_since_last_document_update_time() >
                    // doc->get_milies_since_last_edit_time()) {
                    doc->reload();
    ...
    scroll_bar->connect(
        scroll_bar, &QScrollBar::actionTriggered,
        [this](int action) {
            int value = scroll_bar->value();
            if (main_document_view_has_document()) {
                float offset = doc()->max_y_offset() * value /
                               static_cast<float>(scroll_bar->maximum());
...
MainWidget::~MainWidget() {
    ...
    // todo: use a reference counting pointer for document so we can delete
    // main_doc and helper_doc in DocumentView's destructor, not here. ideally
    // this function should just become:
    //		delete main_document_view;
    //		if(helper_document_view != main_document_view) delete
    // helper_document_view;
    if (main_document_view != nullptr) {
        ...
        // }
        // if (helper_doc != nullptr && helper_doc != main_doc) delete
        // helper_doc;
    }

    if (helper_document_view != nullptr &&
        helper_document_view != main_document_view) {
...
std::wstring MainWidget::get_status_string() {
    ...
    QString status_string = QString::fromStdWString(config.STATUS_BAR_FORMAT);
    ...
    status_string.replace(
        "%{num_pages}",
        QString::number(main_document_view->get_document()->num_pages())
    );
    ...
    if (opengl_widget->get_is_searching(&progress)) {
        ...
        // show the 0th result if there are no results and the index + 1
        // otherwise
        int result_index =
            opengl_widget->get_num_search_results() > 0
                ? opengl_widget->get_current_search_result_index() + 1
                : 0;
        status_string.replace(
            "%{search_results}", " | showing result " +
                                     QString::number(result_index) + " / " +
                                     QString::number(num_search_results)
        );
        if (progress > 0) {
            status_string.replace(
                "%{search_progress}",
    ...
    if (main_document_view != nullptr &&
        main_document_view->get_document() != nullptr &&
    ...
    if (opengl_widget && opengl_widget->get_overview_page()) {
        if (index_into_candidates > 0 && smart_view_candidates.size() > 0) {
            status_string.replace(
                "%{preview_index}",
                " [ preview " + QString::number(index_into_candidates + 1) +
                    " / " + QString::number(smart_view_candidates.size()) + " ]"
            );
        }
    }
...
void MainWidget::handle_escape() {
    ...
    // add high escape priority to overview and search, if any of them are
    // escaped, do not escape any further
    if (opengl_widget) {
...
void MainWidget::keyPressEvent(QKeyEvent* kevent) { key_event(false, kevent); }
    ...
    if (smooth_scroll_mode) {
        if (main_document_view_has_document()) {
            ...
            if (!opengl_widget->get_overview_page()) {
                ...
                main_document_view->set_offset_y(
                    current_offset + smooth_scroll_speed * secs
                );
            } else {
                OverviewState state =
                    opengl_widget->get_overview_page().value();
                // opengl_widget->get_overview_offsets(&overview_offset_x,
                // &overview_offset_y);
                // opengl_widget->set_overview_offsets(overview_offset_x,
                // overview_offset_y + smooth_scroll_speed * secs);
        ...
        if (current_page >= 0) {
            ...
            if (config.IGNORE_WHITESPACE_IN_PRESENTATION_MODE) {
                main_document_view->set_offset_y(
                    ...
                    main_document_view->get_document()->get_page_height(
                        current_page
                    ) / 2
                );
            } else {
...
void MainWidget::move_document(float dx, float dy) {
    if (main_document_view_has_document()) {
        ...
        // float prev_vertical_line_pos =
        // opengl_widget->get_vertical_line_pos(); float new_vertical_line_pos =
        // prev_vertical_line_pos - dy;
...
//         QString	font_size_stylesheet = QString("font-size:
//         %1px").arg(STATUS_BAR_FONT_SIZE); return QString("background-color:
//         %1; color: %2; border: 0; %3").arg(
//             get_color_qml_string(STATUS_BAR_COLOR[0], STATUS_BAR_COLOR[1],
//             STATUS_BAR_COLOR[2]),
//             get_color_qml_string(STATUS_BAR_TEXT_COLOR[0],
//             STATUS_BAR_TEXT_COLOR[1], STATUS_BAR_TEXT_COLOR[2]),
...
void MainWidget::on_config_file_changed(ConfigManager* new_config) {
    ...
    // text_command_line_edit_container->setStyleSheet("background-color: black;
...
void MainWidget::do_synctex_forward_search(
    ...
    int stat = synctex_display_query(
        scanner, latex_file_string.c_str(), line, column, 0
    );
    int target_page = -1;

    if (stat <= 0) {
        stat = synctex_display_query(
            scanner, latex_file_with_redundant_dot_string.c_str(), line, column,
            0
        );
    }
        ...
        if (target_page != -1) {
            ...
            if (highlight_rects.size() == 0) {
                ...
                main_document_view->goto_offset_within_page(
                    {target_page, main_document_view->get_offset_x(),
                     first_rect.value().y0}
                );
            }
        }
...
void MainWidget::update_link_with_opened_book_state(
    ...
    db_manager->update_portal(
        link_owner->get_checksum(), new_state.offset_x, new_state.offset_y,
        new_state.zoom_level, lnk.src_offset_y
    );

    link_to_edit = {};
}

void MainWidget::update_closest_link_with_opened_book_state(
    const OpenedBookState& new_state
) {
    std::optional<Portal> closest_link =
        main_document_view->find_closest_portal();
    if (closest_link) {
...
void MainWidget::invalidate_ui() { is_render_invalidated = true; }
...
void MainWidget::open_document_with_hash(
...
void MainWidget::open_document(
    ...
    if (main_document_view_has_document()) {
        scroll_bar->setSingleStep(
            std::max(MAX_SCROLLBAR / doc()->num_pages() / 10, 1)
        );
...
void MainWidget::open_document_at_location(
    ...
    AbsoluteDocumentPos absolute_pos =
        main_document_view->get_document()->document_to_absolute_pos(
...
void MainWidget::open_document(const DocumentViewState& state) {
    open_document(
        state.document_path, state.book_state.offset_x,
        state.book_state.offset_y, state.book_state.zoom_level
    );
}
...
void MainWidget::handle_command_types(
    std::unique_ptr<Command> new_command, int num_repeats
) {
...
void MainWidget::key_event(bool released, QKeyEvent* kevent) {
    ...
    if (typing_location.has_value()) {
        ...
        if (released == false) {
            ...
            fz_rect character_rect =
                fz_rect_from_quad(typing_location.value().character->quad);
            std::optional<fz_rect> wrong_rect = {};

            if (typing_location.value().previous_character) {
                wrong_rect = fz_rect_from_quad(
                    typing_location.value().previous_character->character->quad
                );
            }

            if (should_focus) {
                main_document_view->set_offset_y(
                    typing_location.value().focus_offset()
                );
            }
    ...
    if (released == false) {
        ...
        if (is_waiting_for_symbol()) {
            ...
            char symb = get_symbol(
                kevent->key(), kevent->modifiers() & Qt::ShiftModifier,
                pending_command_instance->special_symbols()
            );
            if (symb) {
        ...
        bool is_control_pressed = (kevent->modifiers() & Qt::ControlModifier) ||
                                  (kevent->modifiers() & Qt::MetaModifier);
        std::vector<std::unique_ptr<Command>> commands =
            input_handler->handle_key(
                kevent, kevent->modifiers() & Qt::ShiftModifier,
                is_control_pressed, kevent->modifiers() & Qt::AltModifier,
                &num_repeats
            );
...
void MainWidget::handle_right_click(
    WindowPos click_pos,
    bool down,
    ...
    if ((main_document_view->get_document() != nullptr) &&
        ...
        // because we probably don't need them (we are editing our own document
        // after all) we can always use middle click to jump to a destination
        // which is probably what we need anyway
        if (down == true && (!this->synctex_mode)) {
            if (pending_command_instance &&
                (pending_command_instance->get_name() == "goto_mark")) {
...
void MainWidget::handle_left_click(
    WindowPos click_pos,
    bool down,
    bool is_shift_pressed,
    bool is_control_pressed,
    ...
    AbsoluteDocumentPos abs_doc_pos =
        main_document_view->window_to_absolute_document_pos(click_pos);

    auto [normal_x, normal_y] =
        main_document_view->window_to_normalized_window_pos(click_pos);
    ...
    if (rect_select_mode) {
        if (down == true) {
            if (rect_select_end.has_value()) {
                // clicked again after selecting, we should clear the selected
                // rectangle
                clear_selected_rect();
            } else {
            ...
            if (rect_select_begin.has_value() && rect_select_end.has_value()) {
                ...
                if (pending_command_instance) {
                    pending_command_instance->set_rect_requirement(
                        selected_rectangle
                    );
                    advance_command(std::move(pending_command_instance));
                }
    ...
    if (down == true) {
        ...
        PdfViewOpenGLWidget::OverviewSide border_index =
            static_cast<PdfViewOpenGLWidget::OverviewSide>(-1);
        if (opengl_widget->is_window_point_in_overview_border(
                normal_x, normal_y, &border_index
            )) {
            PdfViewOpenGLWidget::OverviewResizeData resize_data;
            resize_data.original_normal_mouse_pos =
                NormalizedWindowPos{normal_x, normal_y};
        ...
        if (opengl_widget->is_window_point_in_overview({normal_x, normal_y})) {
            ...
            move_data.original_normal_mouse_pos =
                NormalizedWindowPos{normal_x, normal_y};
            move_data.original_offsets =
                fvec2{original_offset_x, original_offset_y};
        ...
        if ((!was_overview_mode) && (!mouse_drag_mode) &&
            (manhattan_distance(fvec2(last_mouse_down), fvec2(abs_doc_pos)) > 5
            )) {
            ...
            main_document_view->get_text_selection(
                last_mouse_down, abs_doc_pos, is_word_selecting,
                main_document_view->selected_character_rects, selected_text
            );
...
void MainWidget::push_state(bool update) {
    ...
    if (!main_document_view_has_document())
        return; // we don't add empty document to history
...
void MainWidget::prev_state() {
    ...
    if (current_history_index >= 0) {
        ...
        In order to edit a link, we set the link to edit and jump to the link
        location, when going back, we update the link with the current location
        of document, therefore, we must check to see if a link is being edited
        and if so, we should update its destination position
        */
        if (link_to_edit) {
            ...
            db_manager->update_portal(
                ...
                state.offset_x, state.offset_y, state.zoom_level,
                link_to_edit->src_offset_y
            );
            link_to_edit = {};
        }
...
void MainWidget::handle_click(WindowPos click_pos) {
    ...
    if (opengl_widget->is_window_point_in_overview({normal_x, normal_y})) {
        ...
        auto link = main_document_view->get_document()->get_link_in_pos(
    ...
    selected_highlight_index =
        main_document_view->get_highlight_index_in_pos(click_pos);
...
bool MainWidget::find_location_of_text_under_pointer(
    ...
    auto [page, offset_x, offset_y] =
        main_document_view->window_to_document_pos(pointer_pos);
    int current_page_number = get_current_page_number();

    fz_stext_page* stext_page =
        main_document_view->get_document()->get_stext_with_page_number(page);
    ...
    if (generic_pair) {
        std::vector<DocumentPos> candidates =
            main_document_view->get_document()->find_generic_locations(
                generic_pair.value().first, generic_pair.value().second
            );
    ...
    if (equation_text_on_pointer) {
        std::optional<IndexedData> eqdata_ =
            main_document_view->get_document()->find_equation_with_string(
                equation_text_on_pointer.value(), current_page_number
            );
        if (eqdata_) {
    ...
    if (reference_text_on_pointer) {
        std::optional<IndexedData> refdata_ =
            main_document_view->get_document()->find_reference_with_string(
                reference_text_on_pointer.value()
            );
        if (refdata_) {
...
void MainWidget::mouseReleaseEvent(QMouseEvent* mevent) {
    ...
    bool is_control_pressed = QGuiApplication::keyboardModifiers().testFlag(
        Qt::KeyboardModifier::ControlModifier
    );
    bool is_alt_pressed = QGuiApplication::keyboardModifiers().testFlag(
    ...
    if (mevent->button() == Qt::MouseButton::LeftButton) {
        if (is_shift_pressed) {
            ...
            handle_left_click(
                ...
                is_control_pressed, is_alt_pressed
            );
            if (is_select_highlight_mode &&
                (main_document_view->selected_character_rects.size() > 0)) {
                main_document_view->add_highlight(
                    selection_begin, selection_end, select_highlight_type
                );
                clear_selected_text();
            }
            if (main_document_view->selected_character_rects.size() > 0) {
...
void MainWidget::mousePressEvent(QMouseEvent* mevent) {
    ...
    if (mevent->button() == Qt::MouseButton::LeftButton) {
        handle_left_click(
            {mevent->pos().x(), mevent->pos().y()}, true, is_shift_pressed,
            is_control_pressed, is_alt_pressed
        );
    }
    ...
    if (mevent->button() == Qt::MouseButton::XButton1) {
        handle_command_types(
            command_manager->get_command_with_name("prev_state"), 0
        );
        invalidate_render();
    }
    ...
    if (mevent->button() == Qt::MouseButton::XButton2) {
        handle_command_types(
            command_manager->get_command_with_name("next_state"), 0
        );
        invalidate_render();
    }
}
...
void MainWidget::wheelEvent(QWheelEvent* wevent) {
        ...
        if (opengl_widget->is_window_point_in_overview({normal_x, normal_y})) {
            ...
            if (wevent->angleDelta().y() > 0) {
                ...
                if (is_visual_mark_mode) {
                    command = command_manager->get_command_with_name(
                        "move_visual_mark_up"
                    );
                } else {
                    move_vertical(
                        -72.0f * vertical_move_amount * num_repeats_f
                    );
                    update_scrollbar();
                    return;
                }
            ...
            if (wevent->angleDelta().x() > 0) {
                move_horizontal(
                    -72.0f * horizontal_move_amount * num_repeats_f *
                    inverse_factor
                );
                return;
            }
    ...
    if (is_shift_pressed) {
        ...
        if (wevent->angleDelta().y() > 0) {
            move_horizontal(
                -72.0f * horizontal_move_amount * num_repeats_f * inverse_factor
            );
            return;
        }
        if (wevent->angleDelta().y() < 0) {
...
std::optional<std::wstring> MainWidget::get_paper_name_under_cursor() {
    ...
    auto normal_pos =
        main_document_view->window_to_normalized_window_pos(window_pos);

    if (opengl_widget->is_window_point_in_overview(normal_pos)) {
        auto [doc_page, doc_x, doc_y] =
            opengl_widget->window_pos_to_overview_pos(normal_pos);
        return main_document_view->get_document()->get_paper_name_at_position(
            doc_page, doc_x, doc_y
        );
    } else {
        DocumentPos doc_pos =
            main_document_view->window_to_document_pos(window_pos);
        return main_document_view->get_document()->get_paper_name_at_position(
            doc_pos.page, doc_pos.x, doc_pos.y
        );
    }
}
...
void MainWidget::smart_jump_under_pos(WindowPos pos) {
    ...
    auto [normal_x, normal_y] =
        main_document_view->window_to_normalized_window_pos(pos);

    // if overview page is open and we middle click on a paper name, search it
    // in a search engine
    if (opengl_widget->is_window_point_in_overview({normal_x, normal_y})) {
        auto [doc_page, doc_x, doc_y] =
            opengl_widget->window_pos_to_overview_pos({normal_x, normal_y});
    ...
    auto [page, offset_x, offset_y] =
        main_document_view->window_to_document_pos(pos);
    ...
    if (find_location_of_text_under_pointer(
            pos, &target_page, &target_y_offset
        )) {
        ...
        if (paper_name_on_pointer) {
            handle_paper_name_on_pointer(
                paper_name_on_pointer.value(), is_shift_pressed
            );
        }
    }
    ...
    //    if
    //    (main_document_view->get_document()->find_generic_location(generic_pair.value().first,
    ...
    // if (reference_text_on_pointer) {
    //     std::optional<IndexedData> refdata_ =
    //     main_document_view->get_document()->find_reference_with_string(reference_text_on_pointer.value());
    //     if (refdata_) {
    //         IndexedData refdata = refdata_.value();
    //         long_jump_to_destination(refdata.page, refdata.y_offset);
    //         return;
...
void MainWidget::visual_mark_under_pos(WindowPos pos) {
    ...
    if (document_pos.page != -1) {
        ...
        fz_pixmap* pixmap =
            main_document_view->get_document()->get_small_pixmap(
                document_pos.page
            );
        ...
        auto [abs_doc_x, abs_doc_y] =
            main_document_view->window_to_absolute_document_pos(window_pos);
        main_document_view->set_vertical_line_pos(abs_doc_y);
        int container_line_index =
            main_document_view->get_line_index_of_pos(document_pos);
        if (container_line_index == -1) {
            main_document_view->set_line_index(
                main_document_view->get_line_index_of_vertical_pos()
            );
        } else {
...
bool MainWidget::overview_under_pos(WindowPos pos) {
    ...
    if (main_document_view &&
        (link = main_document_view->get_link_in_pos(pos))) {
    ...
    if (find_location_of_text_under_pointer(
            pos, &autoreference_page, &autoreference_offset, true
        )) {
...
void MainWidget::complete_pending_link(
    const PortalViewState& destination_view_state
) {
...
void MainWidget::long_jump_to_destination(int page, float offset_y) {
    long_jump_to_destination(
        {page, main_document_view->get_offset_x(), offset_y}
    );
}

void MainWidget::long_jump_to_destination(float abs_offset_y) {

    auto [page, _, offset_y] =
...
void MainWidget::long_jump_to_destination(DocumentPos pos) {
    if (!is_pending_link_source_filled()) {
        ...
        // if we press the link button and then click on a pdf link, we
        // automatically link to the link's destination

        PortalViewState dest_state;
        dest_state.document_checksum =
            main_document_view->get_document()->get_checksum();
        dest_state.book_state.offset_x = pos.x;
        dest_state.book_state.offset_y =
            main_document_view->get_page_offset(pos.page) + pos.y;
...
bool MainWidget::focus_on_visual_mark_pos(bool moving_down) {
    ...
    // main_document_view->get_vertical_line_pos(), &window_x, &window_y); if
    // ((window_y < -thresh) || (window_y > thresh)) {
    if (ruler_window_rect.y0 < -1 || ruler_window_rect.y1 > 1) {
        main_document_view->goto_vertical_line_pos();
        return true;
    }
    if ((moving_down && (ruler_window_rect.y0 < -thresh)) ||
        ((!moving_down) && (ruler_window_rect.y1 > thresh))) {
        main_document_view->goto_vertical_line_pos();
        // float distance = (main_document_view->get_view_height() /
        // main_document_view->get_zoom_level()) *
        // VISUAL_MARK_NEXT_PAGE_FRACTION; main_document_view->move_absolute(0,
        // -distance);
        return true;
    }
    return false;
...
void MainWidget::execute_command(
    ...
    while (command_parts_.size() > 0) {
        if ((command_parts_.size() <= 1) ||
            (!command_parts_.at(0).endsWith("\\"))) {
            ...
            QString new_command_part =
                first_part.left(first_part.size() - 1) + " " + second_part;
            command_parts_.pop_front();
    ...
    if (command_parts.size() > 0) {
        ...
        DocumentPos mouse_pos_document =
            main_document_view->window_to_document_pos(mouse_pos);
        ...
        for (int i = 0; i < command_parts.size(); i++) {
            ...
            if (selected_text.size() > 0) {
                ...
                command_parts[i].replace(
                    "%{selection_begin_document}",
                    QString::number(selection_begin_document.page) + " " +
                        QString::number(selection_begin_document.x) + " " +
                        QString::number(selection_begin_document.y)
                );
                auto selection_end_document =
                    main_document_view->get_document()->absolute_to_page_pos(
                        selection_end
                    );
                command_parts[i].replace(
                    "%{selection_end_document}",
                    QString::number(selection_end_document.page) + " " +
                        QString::number(selection_end_document.x) + " " +
                        QString::number(selection_end_document.y)
                );
            }
            command_parts[i].replace(
            ...
            command_parts[i].replace(
                "%{mouse_pos_window}", QString::number(mouse_pos.x) + " " +
                                           QString::number(mouse_pos.y)
            );
            // command_parts[i].replace("%{mouse_pos_window}", QString("%1
            // %2").arg(mouse_pos.x, mouse_pos.y));
            command_parts[i].replace(
                "%{mouse_pos_document}",
                QString::number(mouse_pos_document.page) + " " +
                    QString::number(mouse_pos_document.x) + " " +
                    QString::number(mouse_pos_document.y)
            );
            // command_parts[i].replace("%{mouse_pos_document}", QString("%1 %2
            ...
            command_parts[i].replace(
                "%{sioyek_path}", QCoreApplication::applicationFilePath()
            );
            command_parts[i].replace(
                "%{local_database}",
            ...
            if (get_selected_rect_document(
                    selected_rect_page, selected_rect_rect
                )) {
                QString format_string = "%1,%2,%3,%4,%5";
                QString rect_string =
                    format_string.arg(QString::number(selected_rect_page))
...
void MainWidget::apply_window_params_for_two_window_mode() {
    ...
    get_window_params_for_two_window_mode(
        main_window_size, main_window_move, helper_window_size,
        helper_window_move
    );
...
void MainWidget::open_document(
    ...
    main_document_view->open_document(
        doc_path, invalid_flag, load_prev_state, prev_state,
...
std::vector<fz_rect> MainWidget::get_flat_words(
    ...
    if (flat_word_chars != nullptr) {
        *flat_word_chars =
            main_document_view->get_document()->get_page_flat_word_chars(page);
    }
    return res;
}

std::optional<fz_irect> MainWidget::get_tag_window_rect(
    std::string tag, std::vector<fz_irect>* char_rects
) {
    ...
    if (rect.has_value()) {
        ...
        fz_irect window_rect =
            main_document_view->document_to_window_irect(page, rect.value());
        if (char_rects != nullptr) {
            for (auto c : word_char_rects) {
                char_rects->push_back(
                    main_document_view->document_to_window_irect(page, c)
                );
            }
        }
        return window_rect;
...
bool MainWidget::is_rotated() { return opengl_widget->is_rotated(); }
    ...
    if (main_document_view && (main_document_view->get_document() != nullptr)) {
        if (main_document_view->get_document()->needs_authentication()) {
            if ((pending_command_instance == nullptr) ||
                (pending_command_instance->get_name() != "enter_password")) {
                handle_command_types(
                    command_manager->get_command_with_name("enter_password"), 1
                );
            }
        }
    }
...
void MainWidget::on_new_paper_added(const std::wstring& file_path) {
    if (is_pending_link_source_filled()) {
        ...
        if (helper_document_view) {
            ...
            dst_view_state.book_state.offset_y = -std::abs(
                -helper_view_height / zoom_level / 2 + first_page_height / 2
            );
        }
...
void MainWidget::handle_link_click(const PdfLink& link) {
    ...
    // we usually just want to center the y offset and not the x offset
    // (otherwise for example a link at the right side of the screen will be
    // centered, causing most of screen state to be empty)
...
void MainWidget::save_auto_config() {
    std::wofstream outfile(Config::instance().auto_config_path);
...
std::wstring MainWidget::get_serialized_configuration_string() {
    ...
    opengl_widget->get_overview_offsets(
        &overview_offset[0], &overview_offset[1]
    );
...
std::wstring MainWidget::get_window_configuration_string() {
    ...
    QString config_string_single =
        "single_main_window_size    %1 %2\nsingle_main_window_move     %3 %4";
    ...
    if ((helper_opengl_widget != nullptr) &&
        helper_opengl_widget->isVisible()) {
        helper_window_size_w =
            QString::number(helper_opengl_widget->size().width());
        helper_window_size_h =
            QString::number(helper_opengl_widget->size().height());
...
void MainWidget::handle_close_event() {
    ...
    // we need to delete this here (instead of destructor) to ensure that
    // application closes immediately after the main window is closed
    delete helper_opengl_widget;
    helper_opengl_widget = nullptr;
}

Document* MainWidget::doc() { return main_document_view->get_document(); }
...
void MainWidget::move_visual_mark(int offset) {
    ...
    fz_rect ruler_rect = doc()->get_ith_next_line_from_absolute_y(
        vertical_line_page, prev_line_index, offset, true, &new_line_index,
        &new_page
    );
...
#ifdef Q_OS_WIN
            // the path returned by synctex is formatted in unix style, for
            // example it is something like this in windows:
            // d:/some/path/file.pdf this doesn't work with Vimtex for some
            // reason, so here we have to convert the path separators to windows
            // style and make sure the driver letter is capitalized
            QDir file_path = QDir(file_name);
            QString new_path =
                QDir::toNativeSeparators(file_path.absolutePath());
...
std::optional<DocumentPos> MainWidget::get_overview_position() {
    ...
    if (overview_state_.has_value()) {
        ...
        return main_document_view->get_document()->absolute_to_page_pos(
            {0, overview_state.absolute_offset_y}
        );
        // DocumentPos pos = { overview_state.page, 0.0f,
        // overview_state.offset_y }; return pos;
    }
    return {};
}
...
void MainWidget::add_portal(std::wstring source_path, Portal new_link) {
    if (source_path == main_document_view->get_document()->get_path()) {
        ...
        db_manager->insert_portal(
            checksummer->get_checksum(source_path),
            new_link.dst.document_checksum, new_link.dst.book_state.offset_x,
            new_link.dst.book_state.offset_y,
            new_link.dst.book_state.zoom_level, new_link.src_offset_y
        );
    }
}
    ...
    if (text[0] == '#') {
        ...
        if (parts.size() == 1) {
            ...
            if (schar_rects.size() > 1) {
                ...
                handle_left_click(
                    {erect.x0, (erect.y0 + erect.y1) / 2}, false, false, false,
                    false
                );
                opengl_widget->set_should_highlight_words(false);
            }
        ...
        if (parts.size() == 2) {
            ...
            std::optional<fz_irect> erect_ =
                get_tag_window_rect(parts.at(1).toStdString(), &echar_rects);
            ...
            if ((schar_rects.size() > 0) && (echar_rects.size() > 0)) {
                ...
                handle_left_click(
                    {erect.x0 - w / 2, (erect.y0 + erect.y1) / 2}, false, false,
                    false, false
                );
                opengl_widget->set_should_highlight_words(false);
                ...
                handle_left_click(
                    {srect.x0 + 5, (srect.y0 + srect.y1) / 2}, true, false,
                    false, false
                );
                handle_left_click(
                    {erect.x0 - 5, (erect.y0 + erect.y1) / 2}, false, false,
                    false, false
                );
                opengl_widget->set_should_highlight_words(false);
            }
...
void MainWidget::toggle_scrollbar() {
    ...
    // dirty hack!
    // really the content of this closure should be in toggle_scrollbar method,
    // however, for some unknown reason if we do that, new windows are created
    // very small when toggle_scrollbar is in startup_commands. Strangely the
...
void MainWidget::update_scrollbar() {
    if (main_document_view_has_document()) {
        ...
        int scroll =
            static_cast<int>(MAX_SCROLLBAR * offset / doc()->max_y_offset());
        scroll_bar->setValue(scroll);
    }
}

void MainWidget::handle_portal_overview_update() {
    std::optional<OverviewState> current_state_ =
        opengl_widget->get_overview_page();
    if (current_state_) {
        OverviewState current_state = current_state_.value();
        if (current_state.doc != nullptr) {
            std::optional<Portal> link_ =
                main_document_view->find_closest_portal();
            if (link_) {
...
void MainWidget::goto_overview() {
    if (opengl_widget->get_overview_page()) {
        ...
        if (overview.doc != nullptr) {
            std::optional<Portal> closest_link_ =
                main_document_view->find_closest_portal();
            if (closest_link_) {
                push_state();
            ...
            std::optional<DocumentPos> maybe_overview_position =
                get_overview_position();
...
QString MainWidget::get_font_face_name() {
    ...
    if (config.UI_FONT_FACE_NAME.empty()) {
        ...
        return QString::fromStdWString(config.UI_FONT_FACE_NAME);
    }
}
...
bool MainWidget::get_selected_rect_document(int& out_page, fz_rect& out_rect) {
    ...
    if (absrect) {
        ...
        DocumentPos top_left_document =
            main_document_view->get_document()->absolute_to_page_pos(top_left);
        DocumentPos bottom_right_document =
            main_document_view->get_document()->absolute_to_page_pos(
                bottom_right
            );
...
bool MainWidget::is_rect_visible(int page, fz_rect rect) {
    fz_irect window_rect =
        main_document_view->document_to_window_irect(page, rect);
    if (window_rect.x0 > 0 && window_rect.x1 < main_window_width &&
        window_rect.y0 > 0 && window_rect.y1 < main_window_height) {
        return true;
    } else {
        return false;
    }
...
void MainWidget::set_mark_in_current_location(char symbol) {
    // it is a global mark, we delete other marks with the same symbol from
    // database and add the new mark
    if (isupper(symbol)) {
...
void MainWidget::overview_to_definition() {
    if (!opengl_widget->get_overview_page()) {
        std::vector<DocumentPos> defpos =
            main_document_view->find_line_definitions();
        if (defpos.size() > 0) {
...
void MainWidget::portal_to_definition() {
    ...
    if (defpos.size() > 0) {
        AbsoluteDocumentPos abspos =
            doc()->document_to_absolute_pos(defpos[0], true);
        Portal link;
...
void MainWidget::handle_goto_bookmark() {
    ...
    for (auto bookmark : bookmarks) {
        option_names.push_back(
            config.ITEM_LIST_PREFIX + L" " + bookmark.description
        );
    ...
    int closest_bookmark_index =
        main_document_view->get_document()->find_closest_bookmark_index(
    ...
    set_current_widget(new FilteredTableWindowSelector<float>(
        ...
        [&](float* offset_value) {
            if (offset_value) {
                main_document_view->delete_closest_bookmark_to_offset(
                    *offset_value
                );
            }
        }
...
void MainWidget::handle_goto_bookmark_global() {
    ...
    for (const auto& desc_bm_pair : global_bookmarks) {
        ...
        if (path) {
            ...
            descs.push_back(
                Config::instance().ITEM_LIST_PREFIX + L" " + bm.description
            );
    ...
    set_current_widget(new FilteredTableWindowSelector<BookState>(
        ...
        [&](BookState* book_state) {
            if (book_state) {
                validate_render();
                open_document(
                    book_state->document_path, 0.0f, book_state->offset_y
                );
            }
        },
        this,
        [&](BookState* book_state) {
            if (book_state) {
                db_manager->delete_bookmark(
                    checksummer->get_checksum(book_state->document_path),
                    book_state->offset_y
                );
            }
        }
...
void MainWidget::handle_add_highlight(char symbol) {
    if (main_document_view->selected_character_rects.size() > 0) {
        main_document_view->add_highlight(
            selection_begin, selection_end, symbol
        );
        main_document_view->selected_character_rects.clear();
        selected_text.clear();
    } else if (selected_highlight_index != -1) {
        Highlight new_highlight = main_document_view->get_highlight_with_index(
            selected_highlight_index
        );
        main_document_view->delete_highlight_with_index(selected_highlight_index
        );
        main_document_view->add_highlight(
            new_highlight.selection_begin, new_highlight.selection_end, symbol
        );
...
void MainWidget::handle_goto_highlight() {
    ...
    int closest_highlight_index =
        main_document_view->get_document()->find_closest_highlight_index(
            highlights, main_document_view->get_offset_y()
        );
            ...
            set_current_widget(new FilteredTreeSelector(
                ...
                [&](const std::vector<int>& indices) {
                    TocNode* toc_node = get_toc_node_from_indices(
                        main_document_view->get_document()->get_toc(), indices
                    );
                    if (toc_node) {
                        validate_render();
...
void MainWidget::handle_new_window() {
    MainWidget* new_widget = new MainWidget(
        mupdf_context, db_manager, document_manager, config_manager,
        command_manager, input_handler, checksummer, should_quit
    );
...
std::optional<std::pair<int, fz_link*>> MainWidget::get_selected_link(
    ...
    if (config.ALPHABETIC_LINK_TAGS || is_string_numeric(text)) {
        ...
        if ((link_index >= 0) &&
            (link_index < static_cast<int>(visible_page_links.size()))) {
...
void MainWidget::handle_portal_to_link(const std::wstring& text) {
    ...
    if (selected_link_) {
        ...
        auto src_abspos =
            doc()->document_to_absolute_pos(link_source_document_pos, true);
...
void MainWidget::handle_portal_to_overview() {
    std::optional<DocumentPos> maybe_overview_position =
        get_overview_position();
    if (maybe_overview_position.has_value()) {
        AbsoluteDocumentPos abs_pos =
            doc()->document_to_absolute_pos(maybe_overview_position.value());
        ...
        new_portal.dst.book_state.zoom_level =
            main_document_view->get_zoom_level();
...
void MainWidget::handle_overview_to_portal() {
    if (opengl_widget->get_overview_page()) {
        ...
        std::optional<Portal> portal_ =
            main_document_view->find_closest_portal();
        if (portal_) {
            Portal portal = portal_.value();
            auto destination_path =
                checksummer->get_path(portal.dst.document_checksum);
            if (destination_path) {
                Document* doc =
                    document_manager->get_document(destination_path.value());
                if (doc) {
                    overview_state.absolute_offset_y =
...
void MainWidget::handle_toggle_typing_mode() {
    if (!typing_location.has_value()) {
        ...
        opengl_widget->set_typing_rect(
            charaddr.page, fz_rect_from_quad(charaddr.character->quad), {}
        );
...
void MainWidget::handle_delete_highlight_under_cursor() {
    ...
    int sel_highlight =
        main_document_view->get_highlight_index_in_pos(window_pos);
    if (sel_highlight != -1) {
