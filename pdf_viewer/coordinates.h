...
struct AbsoluteDocumentPos {
    float x;
    // this is the concatenated y-coordinate of the current page (sum of all
    // page heights up to current location)
    float y;
};
...
struct Vec {
    ...
    T width() { return values[0]; }

    T height() { return values[1]; }
