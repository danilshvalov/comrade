...
class PdfViewOpenGLWidget : public QOpenGLWidget,
  ...
  protected:
    ...
    void render_highlight_window(
        GLuint program, fz_rect window_rect, bool draw_border = true
    );
    void render_line_window(
        GLuint program,
        float vertical_pos,
        std::optional<fz_rect> ruler_rect = {}
    );
    void render_highlight_absolute(
        GLuint program, fz_rect absolute_document_rect, bool draw_border = true
    );
    ...
    void draw_stencil_rects(
        const std::vector<fz_rect>& rects,
        bool is_window_rect = false,
        int page = -1
    );
  ...
  public:
    ...
    void register_on_link_edit_listener(
    ...
    fz_rect get_overview_rect_pixel_perfect(
    ...
    bool is_window_point_in_overview_border(
    ...
    void bind_program();
    void cancel_search();
    // void window_pos_to_overview_pos(float window_x, float window_y, float*
    // doc_offset_x, float* doc_offset_y, int* doc_page);
