...
class PdfRenderer : public QObject {
    ...
    // Since the context should only be used from the thread that initialized
    // it, we can not simply clone the context in the initializer because the
    // initializer is called from the main thread. Instead, we just save a
    // pointer to the context in the initializer and then clone the context when
    // run() is called in the worker
    // thread.
