...
float DocumentView::get_zoom_level() { return zoom_level; }
...
float DocumentView::get_offset_x() { return offset_x; }

float DocumentView::get_offset_y() { return offset_y; }

AbsoluteDocumentPos DocumentView::get_offsets() { return {offset_x, offset_y}; }

int DocumentView::get_view_height() { return view_height; }

int DocumentView::get_view_width() { return view_width; }

void DocumentView::set_null_document() { current_document = nullptr; }
...
int DocumentView::get_highlight_index_in_pos(WindowPos window_pos) {
    ...
    if (current_document->can_use_highlights()) {
        const std::vector<Highlight>& highlights =
            current_document->get_highlights();

        for (size_t i = 0; i < highlights.size(); i++) {
            for (size_t j = 0; j < highlights[i].highlight_rects.size(); j++) {
                if (fz_is_point_inside_rect(
                        pos, highlights[i].highlight_rects[j]
                    )) {
                    return i;
                }
            }
...
void DocumentView::add_highlight(
    ...
    if (current_document) {
        ...
        if (selected_text.size() > 0) {
            current_document->add_highlight(
                selected_text, merged_characters, selection_begin,
                selection_end, type
            );
        }
    }
...
// void DocumentView::absolute_to_window_pos_pixels(float absolute_x, float
// absolute_y, float* window_x, float* window_y) {
//
// }

void DocumentView::absolute_to_window_pos(
    float absolute_x, float absolute_y, float* window_x, float* window_y
) {
...
fz_irect DocumentView::document_to_window_irect(int page, fz_rect doc_rect) {
    ...
    WindowPos bottom_left =
        document_to_window_pos_in_pixels({page, doc_rect.x0, doc_rect.y0});
    WindowPos top_right =
        document_to_window_pos_in_pixels({page, doc_rect.x1, doc_rect.y1});
...
fz_rect DocumentView::document_to_window_rect(int page, fz_rect doc_rect) {
    ...
    NormalizedWindowPos p0 =
        document_to_window_pos({page, doc_rect.x0, doc_rect.y0});
    NormalizedWindowPos p1 =
        document_to_window_pos({page, doc_rect.x1, doc_rect.y1});
...
fz_rect DocumentView::document_to_window_rect_pixel_perfect(
    int page, fz_rect doc_rect, int pixel_width, int pixel_height
) {
...
NormalizedWindowPos DocumentView::window_to_normalized_window_pos(
    ...
    float normal_y = 2 *
                     (static_cast<float>(window_pos.y) - view_height / 2.0f) /
                     view_height;
...
void DocumentView::goto_mark(char symbol) {
    if (current_document) {
        ...
        if (current_document->get_mark_location_if_exists(
                symbol, &new_y_offset
            )) {
...
void DocumentView::goto_end() {
    if (current_document) {
        ...
        set_offset_y(
            current_document->get_accum_page_height(last_page_index) +
            current_document->get_page_height(last_page_index)
        );
    }
}
...
void DocumentView::goto_left_smart() {
    ...
    float page_width = current_document->get_page_size_smart(
        true, get_center_page_number(), &left_ratio, &right_ratio,
        &normal_page_width
    );
...
float DocumentView::set_zoom_level(
    float zl, bool should_exit_auto_resize_mode
) {
...
float DocumentView::zoom_in_cursor(WindowPos mouse_pos, float zoom_factor) {
    ...
    AbsoluteDocumentPos prev_doc_pos =
        window_to_absolute_document_pos(mouse_pos);

    float res = zoom_in(zoom_factor);

    AbsoluteDocumentPos new_doc_pos =
        window_to_absolute_document_pos(mouse_pos);

    move_absolute(
        -prev_doc_pos.x + new_doc_pos.x, prev_doc_pos.y - new_doc_pos.y
    );
...
//		get_current_page_number(), text, &search_results, &percent_done,
//&is_searching, &search_results_mutex);
// }
...
void DocumentView::get_visible_pages(
    int window_height, std::vector<int>& visible_pages
) {
    if (!current_document)
        return;
    ...
    current_document->get_visible_pages(
        window_y_range_begin, window_y_range_end, visible_pages
    );
...
void DocumentView::open_document(
    ...
    if (prev_state) {
        ...
        std::optional<std::string> checksum =
            checksummer->get_checksum_fast(canonical_path);
        std::vector<OpenedBookState> prev_state;
        if (checksum &&
            db_manager->select_opened_book(checksum.value(), prev_state)) {
            if (prev_state.size() > 1) {
                std::cerr << "more than one file with one path, this should "
                             "not happen!"
                          << std::endl;
            }
        }
...
// void DocumentView::goto_toc_link(std::variant<PageTocLink, ChapterTocLink>
// toc_link) { 	int page = -1;
//
...
void DocumentView::fit_to_page_height(bool smart) {
    ...
    int page_height = current_document->get_page_size_smart(
        false, cp, &top_ratio, &bottom_ratio, &normal_page_height
    );
...
void DocumentView::fit_to_page_width(bool smart, bool ratio) {
    ...
    if (smart) {
        ...
        float left_ratio, right_ratio;
        int normal_page_width;
        int page_width = current_document->get_page_size_smart(
            true, cp, &left_ratio, &right_ratio, &normal_page_width
        );
        ...
        set_offset_x(0);
        set_zoom_level(
            static_cast<float>(virtual_view_width) / page_width, true
        );
    }
}
...
void DocumentView::fit_to_page_height_width_minimum() {
    ...
    if (IGNORE_STATUSBAR_IN_PRESENTATION_MODE) {
        ...
        y_zoom_level =
            (static_cast<float>(view_height) - get_status_bar_height()) /
            page_height;
    }
...
void DocumentView::persist() {
    ...
    db_manager->update_book(
        current_document->get_checksum(), zoom_level, offset_x, offset_y
    );
}

int DocumentView::get_current_chapter_index() {
    const std::vector<int>& chapter_pages =
        current_document->get_flat_toc_pages();
...
void DocumentView::get_page_chapter_index(
    int page, std::vector<TocNode*> nodes, std::vector<int>& res
) {
...
std::optional<fz_rect> DocumentView::get_ruler_rect() { return ruler_rect; }
...
void DocumentView::get_text_selection(
    AbsoluteDocumentPos selection_begin,
    AbsoluteDocumentPos selection_end,
    bool
    ...
    if (current_document) {
        current_document->get_text_selection(
            selection_begin, selection_end, is_word_selection,
            selected_characters, selected_text
        );
    }
}
...
void DocumentView::rotate() {
    ...
    float new_offset = current_document->get_accum_page_height(current_page) +
                       current_document->get_page_height(current_page) / 2;
    set_offset_y(new_offset);
}

void DocumentView::goto_top_of_page() {
    int current_page = get_center_page_number();
    float offset_y = get_document()->get_accum_page_height(current_page) +
                     static_cast<float>(view_height) / 2.0f / zoom_level;
    set_offset_y(offset_y);
}

void DocumentView::goto_bottom_of_page() {
    int current_page = get_center_page_number();
    float offset_y = get_document()->get_accum_page_height(current_page + 1) -
                     static_cast<float>(view_height) / 2.0f / zoom_level;

    if (current_page + 1 == current_document->num_pages()) {
        offset_y = get_document()->get_accum_page_height(current_page) +
                   get_document()->get_page_height(current_page) -
                   static_cast<float>(view_height) / 2.0f / zoom_level;
    }
    set_offset_y(offset_y);
}
...
void DocumentView::set_line_index(int index) { line_index = index; }

int DocumentView::get_line_index_of_vertical_pos() {
    DocumentPos line_doc_pos =
        current_document->absolute_to_page_pos({0, get_ruler_pos()});
...
std::vector<DocumentPos> DocumentView::find_line_definitions() {
    // todo: remove duplicate code from this function, this just needs to find
    // the location of the
    ...
    if (line_index > 0) {
        ...
        if ((size_t)line_index < lines.size()) {
            ...
            std::optional<PdfLink> pdf_link =
                current_document->get_link_in_page_rect(
                    get_center_page_number(), line_rects[line_index]
                );
            if (pdf_link.has_value()) {
                auto parsed_uri =
                    parse_uri(mupdf_context, pdf_link.value().uri);
                result.push_back(
                    {parsed_uri.page - 1, parsed_uri.x, parsed_uri.y}
                );
                return result;
            }
            ...
            for (auto reference_text : reference_texts) {
                reference_text =
                    reference_text.substr(1, reference_text.size() - 2);
                auto index =
                    current_document->find_reference_with_string(reference_text
                    );
                if (index.has_value()) {
                    reference_positions.push_back(
                        {index.value().page, 0, index.value().y_offset}
                    );
                }
            }
            for (auto equation_text : equation_texts) {
                equation_text =
                    equation_text.substr(1, equation_text.size() - 2);
                auto index = current_document->find_equation_with_string(
                    equation_text, get_center_page_number()
                );
                if (index.has_value()) {
            ...
            std::vector<std::vector<DocumentPos>*> res_vectors = {
                &equation_positions, &reference_positions, &generic_positions};
            int max_size = 0;

            for (const auto& vec : res_vectors) {
...
bool DocumentView::get_is_auto_resize_mode() { return is_auto_resize_mode; }
...
void DocumentView::get_visible_links(
    ...
    for (auto page : visible_pages) {
        ...
        while (link) {
            ...
            if ((window_rect.x0 >= -1) && (window_rect.x0 <= 1) &&
                (window_rect.y0 >= -1) && (window_rect.y0 <= 1)) {
