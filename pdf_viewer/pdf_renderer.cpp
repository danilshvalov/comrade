...
PdfRenderer::PdfRenderer(
    ...
    : context_to_clone(context_to_clone),
...
void PdfRenderer::start_threads() {
    ...
    for (int i = 0; i < num_threads; i++) {
        worker_threads.push_back(std::thread([&, i]() { run(i); }));
    }
    search_thread = std::thread([&]() { run_search(num_threads); });
}
    ...
    if (path.size() > 0) {
        ...
        for (auto& cached_resp : cached_responses) {
            if ((cached_resp.request == req) &&
                (cached_resp.invalid == false)) {
                cached_resp.last_access_time =
                    QDateTime::currentMSecsSinceEpoch();

                if (page_width)
                    *page_width = cached_resp.width;
                if (page_height)
                    *page_height = cached_resp.height;

                // We can only use OpenGL in the main thread, so we can not
                // upload the rendered pixmap into a texture in the worker
                // thread, so whenever we get a rendered page in the main
                // thread, we initialize its OpenGL texture if it is not
                // initialized already
                if (cached_resp.texture != 0) {
                    ...
                    // OpenGL usually expects powers of two textures and since
                    // our pixmaps dimensions are often not powers of two, we
                    // set the unpack alignment to 1 (no alignment)

                    glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
                    glTexImage2D(
                        GL_TEXTURE_2D, 0, GL_RGB, cached_resp.pixmap->w,
                        cached_resp.pixmap->h, 0, GL_RGB, GL_UNSIGNED_BYTE,
                        cached_resp.pixmap->samples
                    );
                    glPixelStorei(GL_UNPACK_ALIGNMENT, 4);

                    // don't need the pixmap anymore
                    pixmap_drop_mutex[cached_resp.thread].lock();
                    pixmaps_to_drop[cached_resp.thread].push_back(
                        cached_resp.pixmap
                    );
        ...
        if (result == 0) {
            ...
            return try_closest_rendered_page(
                path, page, zoom_level, page_width, page_height
            );
        }
        return result;
    }
...
GLuint PdfRenderer::try_closest_rendered_page(
    ...
    /*
    If the requested page is not available, we try to find the rendered page
    with the closest possible zoom level to our request and return that instead
    */
    cached_response_mutex.lock();
    ...
    for (const auto& cached_resp : cached_responses) {
        if ((cached_resp.request.path == doc_path) &&
            (cached_resp.request.page == page) && (cached_resp.texture != 0)) {
            float diff = cached_resp.request.zoom_level - zoom_level;
            if (diff <= min_diff) {
                min_diff = diff;
                best_texture = cached_resp.texture;
                if (page_width)
                    *page_width = static_cast<int>(
                        cached_resp.width * zoom_level /
                        cached_resp.request.zoom_level
                    );
                if (page_height)
                    *page_height = static_cast<int>(
                        cached_resp.height * zoom_level /
                        cached_resp.request.zoom_level
                    );
            }
        }
...
void PdfRenderer::delete_old_pages(bool force_all, bool invalidate_all) {
    /*
    Deletes old cached pages. This function should only be called from the main
    thread. OpenGL textures are released immediately but pixmaps should be freed
    from the thread that created them, so we add old pixmaps to
    pixmaps_to_delete and delete them later from the worker thread
    */
    cached_response_mutex.lock();
    ...
    for (size_t i = 0; i < cached_responses.size(); i++) {
        cached_response_times.push_back(
    ...
    if (force_all) {
        ...
        std::nth_element(
            cached_response_times.begin(),
            cached_response_times.begin() + N - 1, cached_response_times.end()
        );
...
void PdfRenderer::run_search(int thread_index) {
    ...
    while (!(*should_quit_pointer)) {
        if (pending_search_request.has_value()) {
            ...
            fz_document* doc =
                get_document_with_path(thread_index, mupdf_context, req.path);
            ...
            if (page_begin < 0)
                page_begin = 0;
            if (page_begin > num_pages_in_document - 1)
                page_begin = num_pages_in_document - 1;
            if (page_end < 0)
                page_end = 0;
            if (page_end > num_pages_in_document - 1)
                page_end = num_pages_in_document - 1;
            //}
            ...
            *req.is_searching = true;
            ...
            while (num_handled_pages < num_pages &&
                   (!pending_search_request.has_value()) &&
                   (!(*should_quit_pointer))) {
                num_handled_pages++;
                ...
                if (num_results > 0) {
                    ...
                    for (int j = 0; j < num_results; j++) {
                        if (hit_mark[j] == 1) {
                            ...
                            req.search_results->push_back(SearchResult{
                                std::vector<fz_rect>(), i});
                        }
                        req.search_results->back().rects.push_back(
                            fz_rect_from_quad(hitboxes[j])
                        );
                    }
                    req.search_results_mutex->unlock();
                }
        ...
        if (fz_needs_password(mupdf_context, ret_val)) {
            if (document_passwords.find(path) != document_passwords.end()) {
                fz_authenticate_password(
                    mupdf_context, ret_val, document_passwords[path].c_str()
                );
            }
        }
...
void PdfRenderer::delete_old_pixmaps(
    int thread_index, fz_context* mupdf_context
) {
...
void PdfRenderer::run(int thread_index) {
    ...
    while (!(*should_quit_pointer)) {
        ...
        if (*should_quit_pointer)
            break;
        // cout << "worker thread running ... pending requests: " <<
        // pending_render_requests.size() << endl;

        RenderRequest req =
            pending_render_requests[pending_render_requests.size() - 1];
        ...
        for (const auto& cached_rep : cached_responses) {
            if ((cached_rep.request == req) && (cached_rep.invalid == false))
        ...
        if (!is_already_rendered) {
            ...
            fz_try(mupdf_context) {
                fz_matrix transform_matrix = fz_pre_scale(
                    fz_identity, req.zoom_level * display_scale,
                    req.zoom_level * display_scale
                );
                fz_document* doc = get_document_with_path(
                    thread_index, mupdf_context, req.path
                );
                ...
                // }
                // else {
                rendered_pixmap = fz_new_pixmap_from_page_number(
                    mupdf_context, doc, req.page, transform_matrix,
                    fz_device_rgb(mupdf_context), 0
                );
                //}
