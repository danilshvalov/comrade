...
// Public interface
namespace fts {
static bool fuzzy_match_simple(const char* pattern, const char* str);
static bool fuzzy_match(const char* pattern, const char* str, int& outScore);
...
static bool fuzzy_match(
    ...
    return fuzzy_internal::fuzzy_match_recursive(
        pattern, str, outScore, str, nullptr, matches, maxMatches, 0,
    ...
    while (*pattern != '\0' && *str != '\0') {
        ...
        if (tolower(*pattern) == tolower(*str)) {
            ...
            if (fuzzy_match_recursive(
                    pattern, str + 1, recursiveScore, strBegin, matches,
