...
void Document::fill_highlight_rects(fz_context* ctx, fz_document* doc_) {
    // Fill the `highlight_rects` attribute of all highlights with a vector of
    // merged highlight rects of individual characters in the highlighted area
    // (we merge the rects of characters in a single line which makes the result
    // look nicer)
...
void Document::add_highlight(
    ...
    db_manager->insert_highlight(
        get_checksum(), desc, selection_begin.x, selection_begin.y,
        selection_end.x, selection_end.y, highlight.type
    );
}

bool Document::get_is_indexing() { return is_indexing; }

void Document::add_portal(Portal portal, bool insert_into_database) {
    portals.push_back(portal);
    if (insert_into_database) {
        db_manager->insert_portal(
            get_checksum(), portal.dst.document_checksum,
...
int Document::find_closest_bookmark_index(
...
int Document::find_closest_highlight_index(
...
void Document::delete_closest_bookmark(float to_y_offset) {
    ...
    if (closest_index > -1) {
        db_manager->delete_bookmark(
            get_checksum(), bookmarks[closest_index].y_offset
        );
...
void Document::delete_highlight_with_index(int index) {
    ...
    db_manager->delete_highlight(
        get_checksum(), highlight_to_delete.selection_begin.x,
        highlight_to_delete.selection_begin.y,
        highlight_to_delete.selection_end.x, highlight_to_delete.selection_end.y
    );
...
std::optional<Portal>
Document::find_closest_portal(float to_offset_y, int* index) {
...
void Document::delete_closest_portal(float to_offset_y) {
    ...
    if (find_closest_portal(to_offset_y, &closest_index)) {
        db_manager->delete_link(
            get_checksum(), portals[closest_index].src_offset_y
        );
...
std::vector<BookMark> Document::get_sorted_bookmarks() const {
    ...
    std::sort(
        res.begin(), res.end(),
        [](const BookMark& lhs, const BookMark& rhs) {
...
const std::vector<Highlight> Document::get_highlights_sorted(char type) const {
    ...
    std::sort(
        res.begin(), res.end(),
        [](const Highlight& hl1, const Highlight& hl2) {
...
float Document::get_page_size_smart(
    bool width,
    int page_index,
    float* left_ratio,
    float* right_ratio,
    ...
    // project the small pixmap into a histogram (the `width` parameter
    // determines whether we do this horizontally or vertically)
    ...
    while ((start_index < end_index) &&
           (histogram[start_index++] < nonzero_threshold))
        ;
    while ((start_index < end_index) &&
           (histogram[end_index--] < nonzero_threshold))
        ;
...
fz_outline* Document::get_toc_outline() {
    ...
    fz_try(context) { res = fz_load_outline(context, doc); }
    fz_catch(context) {
...
void Document::convert_toc_tree(
    fz_outline* root, std::vector<TocNode*>& output
) {
...
bool Document::open(
    ...
    if (doc == nullptr) {
        ...
        // std::cerr << "warning! calling open() on an open document" <<
        // std::endl;
        return true;
    }
}
...
void Document::get_visible_pages(
    float doc_y_range_begin,
    float doc_y_range_end,
    ...
    for (size_t i = 0; i < page_heights.size(); i++) {
        ...
        if (range_intersects(
                doc_y_range_begin, doc_y_range_end, page_begin, page_end
            )) {
...
void Document::load_page_dimensions(bool force_load_now) {
    ...
    // initially assume all pages have the same dimensions, correct these
    // heights when the background thread is done
    if (n > 0) {
        fz_try(context) {
...
int Document::num_pages() {
    ...
    fz_catch(context) { std::cerr << "could not count pages" << std::endl; }
    return pages;
}
...
const std::unordered_map<std::wstring, Document*>&
DocumentManager::get_cached_documents() {
...
fz_stext_page* Document::get_stext_with_page_number(
    fz_context* ctx, int page_number, fz_document* doc_
) {
...
int Document::get_page_offset() { return page_offset; }

void Document::set_page_offset(int new_offset) { page_offset = new_offset; }

fz_rect
Document::absolute_to_page_rect(const fz_rect& absolute_rect, int* page) {
    int page_number = -1;
...
DocumentPos Document::absolute_to_page_pos(AbsoluteDocumentPos absp) {
    ...
    int i = (std::lower_bound(
                 accum_page_heights.begin(), accum_page_heights.end(), absp.y
             ) -
...
int Document::get_offset_page_number(float y_offset) {
    ...
    auto it = std::lower_bound(
        accum_page_heights.begin(), accum_page_heights.end(), y_offset
    );

    // std::lower_bound returns an iterator pointing to the first element of the
    // vector not less than y_offset, but we are looking for the last element of
    // vector that is less than y_offset
...
void Document::index_document(bool* invalid_flag) {
    ...
    if (this->document_indexing_thread.has_value()) {
        // if we are already indexing figures, we should wait for the previous
        // thread to finish
        ...
        fz_try(context_) {
            ...
            if (document_needs_password) {
                fz_authenticate_password(
                    context_, doc_, correct_password.c_str()
                );
            }
            for (int i = 0; i < n; i++) {
                ...
                // we don't use get_stext_with_page_number here on purpose
                // because it would lead to many unnecessary allocations
                fz_stext_page* stext_page = fz_new_stext_page_from_page_number(
                    context_, doc_, i, nullptr
                );
...
Document::find_reference_with_string(std::wstring reference_name) {
...
std::optional<std::wstring> Document::get_regex_match_at_position(
    ...
    for (size_t i = 0; i < match_ranges.size(); i++) {
        ...
        for (int index = start_index; index <= end_index; index++) {
            if (fz_contains_rect(
                    fz_rect_from_quad(flat_chars[index]->quad), selected_rect
                )) {
...
std::vector<DocumentPos> Document::find_generic_locations(
    ...
    for (size_t i = 0; i < generic_indices.size(); i++) {
        ...
        if (parts.size() == 2) {
            ...
            if (current_name == name) {
                ...
                DocumentPos pos{
                    generic_indices[i].page, 0, generic_indices[i].y_offset};
    ...
    auto by_score = [](const std::pair<int, DocumentPos>& a,
                       const std::pair<int, DocumentPos>& b) {
...
Document::get_generic_link_name_at_position(
    const std::vector<fz_stext_char*>& flat_chars,
    float offset_x,
    float offset_y
) {
...
std::optional<std::wstring> Document::get_reference_text_at_position(
    ...
    bool done =
        false; // true when whe reach a delimeter after the click location
...
void Document::get_text_selection(
    AbsoluteDocumentPos selection_begin,
    AbsoluteDocumentPos selection_end,
    bool
...
void Document::get_text_selection(
    ...
    for (int i = page_begin; i <= page_end; i++) {
        ...
        if (i == page_begin) {
            char_begin = find_closest_char_to_document_point(
                flat_chars, page_point1, &location_index1
            );
        }
        if (i == page_end) {
            char_end = find_closest_char_to_document_point(
                flat_chars, page_point2, &location_index2
            );
        }
        ...
        for (auto current_char : flat_chars) {
            if (!is_word_selection) {
                ...
                if (word_selecting == false &&
                    is_separator(char_begin, current_char)) {
                    selected_text.clear();
                    selected_characters.clear();
                    continue;
                }
                ...
                if (word_selecting == true &&
                    is_separator(char_end, current_char) &&
                    selecting == false) {
...
void Document::embed_annotations(std::wstring new_file_path) {
    ...
    for (auto highlight : doc_highlights) {
        ...
        for (auto absrect : merged_characters) {
            selected_characters_page_rects.push_back(
                absolute_to_page_rect(absrect, nullptr)
            );
        }
        ...
        pdf_set_annot_quad_points(
            context, highlight_annot, selected_character_quads.size(),
            &selected_character_quads[0]
        );
    ...
    for (auto bookmark : doc_bookmarks) {
        ...
        pdf_set_annot_contents(
            context, bookmark_annot, encoded_bookmark_text.c_str()
        );
...
Document::get_text_at_position(int page, float offset_x, float offset_y) {
...
std::optional<std::wstring>
Document::get_paper_name_at_position(int page, float offset_x, float offset_y) {
    fz_stext_page* stext_page = get_stext_with_page_number(page);
    std::vector<fz_stext_char*> flat_chars;
    get_flat_chars_from_stext_page(stext_page, flat_chars);
...
std::vector<std::vector<fz_rect>> Document::get_page_flat_word_chars(int page) {
    // warning: this function should only be called after get_page_flat_words
    // has already cached the chars
...
Document::get_next_highlight(float abs_y, char type, int offset) const {
...
std::optional<Highlight>
Document::get_prev_highlight(float abs_y, char type, int offset) const {
...
bool Document::needs_password() { return document_needs_password; }
...
Document::get_link_in_pos(int page, float doc_x, float doc_y) {
    if (!doc)
        return {};
...
int Document::add_stext_page_to_created_toc(
    ...
    auto add_toc_node = [&](TocNode* node) {
        if (toc_node_stack.size() == 0) {
            ...
            while ((toc_node_stack.size() > 0) &&
                   (!is_title_parent_of(
                       toc_node_stack[toc_node_stack.size() - 1]->title,
                       node->title, &are_same
                   )) &&
                   (!are_same)
            ) { // ignore items with the same name as current parent (happens
                // when title of chapter is written on top of pages)
            ...
            if (toc_node_stack.size() > 0) {
                toc_node_stack[toc_node_stack.size() - 1]->children.push_back(
                    node
                );
            } else {
                toc_node_stack.push_back(node);
...
AbsoluteDocumentPos
Document::document_to_absolute_pos(DocumentPos doc_pos, bool center_mid) {
...
fz_rect Document::document_to_absolute_rect(
...
// void Document::get_ith_next_line_from_absolute_y(float absolute_y, int i,
// bool cont, float* out_begin, float* out_end) { 	auto [page, doc_x, doc_y]
// =
// absolute_to_page_pos({ 0, absolute_y });
//
//	fz_pixmap* pixmap = get_small_pixmap(page);
//	std::vector<unsigned int> hist =
// get_max_width_histogram_from_pixmap(pixmap); 	std::vector<unsigned
// int> line_locations; 	std::vector<unsigned int> line_locations_begins;
//	get_line_begins_and_ends_from_histogram(hist, line_locations_begins,
// line_locations); 	int small_doc_y = static_cast<int>(doc_y *
// SMALL_PIXMAP_SCALE);
//
//	int index = find_nth_larger_element_in_sorted_list(line_locations,
// static_cast<unsigned int>(small_doc_y - 0.3f), i);
//
//	if (index > -1) {
//		int best_vertical_loc = line_locations[index];
//		int best_vertical_loc_begin = line_locations_begins[index];
//
//		float best_vertical_loc_doc_pos = best_vertical_loc /
// SMALL_PIXMAP_SCALE; 		float best_vertical_loc_begin_doc_pos =
// best_vertical_loc_begin / SMALL_PIXMAP_SCALE;
//
//		float abs_doc_y = document_to_absolute_y(page,
// best_vertical_loc_doc_pos); 		float abs_doc_begin_y =
// document_to_absolute_y(page, best_vertical_loc_begin_doc_pos);
// *out_begin = abs_doc_begin_y; 		*out_end = abs_doc_y;
//	}
//	else {
//		if (!cont) {
...
//+ 1; 			next_page = get_offset_page_number(absolute_y) + 1;
//if (next_page < num_pages()) { 				return
// get_ith_next_line_from_absolute_y(get_accum_page_height(next_page) + 0.5, 1,
// false, out_begin, out_end);
//			}
//		}
//		else {
...
//			if (next_page > 0) {
//				return
// get_ith_next_line_from_absolute_y(get_accum_page_height(next_page) - 0.5f,
// -1, false, out_begin, out_end);
//			}
//		}
//		*out_begin = absolute_y;
//		*out_end = absolute_y;
...
// void Document::get_ith_next_line_from_absolute_y(float absolute_y, int i,
// bool cont, float* out_begin, float* out_end) {
fz_rect Document::get_ith_next_line_from_absolute_y(
    int page, int line_index, int i, bool cont, int* out_index, int* out_page
) {
    // auto [page, doc_x, doc_y] = absolute_to_page_pos({ 0, absolute_y });

    auto line_rects = get_page_lines(page);
    // int index = 0;
    // while ((index < line_rects.size()) && (line_rects[index].y0 <
    // absolute_y)) { 	index++;
    // }
    if (line_index < 0) {
        ...
        *out_index = new_index;
        ...
        if (i > 0) {
            ...
            if (next_page < num_pages()) {
                return get_ith_next_line_from_absolute_y(
                    next_page, 0, 0, false, out_index, out_page
                );
            }
        } else {
            int next_page = page - 1;
            if (next_page >= 0) {
                return get_ith_next_line_from_absolute_y(
                    next_page, -1, 0, false, out_index, out_page
                );
            }
        }
        *out_page = page;
    ...
    //		if (next_page < num_pages()) {
    //			return
    // get_ith_next_line_from_absolute_y(get_accum_page_height(next_page) + 0.5,
    // 1, false, out_begin, out_end);
    //		}
    //	}
    //	else {
    ...
    //		if (next_page > 0) {
    //			return
    // get_ith_next_line_from_absolute_y(get_accum_page_height(next_page) -
    // 0.5f, -1, false, out_begin, out_end);
    //		}
    //	}
    //	*out_begin = absolute_y;
    //	*out_end = absolute_y;
...
Document::get_page_lines(int page, std::vector<std::wstring>* out_line_texts) {
    ...
    if (cached_page_line_rects.find(page) != cached_page_line_rects.end()) {
        ...
        if (stext_page && stext_page->first_block &&
            ...
            get_line_begins_and_ends_from_histogram(
                hist, line_locations_begins, line_locations
            );
            ...
            for (size_t i = 0; i < line_locations_begins.size(); i++) {
                ...
                line_rect.x1 =
                    static_cast<float>(pixmap->w) / SMALL_PIXMAP_SCALE -
                    page_widths[page] / 2;
...
std::vector<SearchResult> Document::search_text(
    ...
    auto it = std::search(
        super_fast_search_index.begin(), super_fast_search_index.end(), searcher
    );

    for (; it != super_fast_search_index.end();
         it = std::search(it + 1, super_fast_search_index.end(), searcher)) {
...
std::vector<SearchResult> Document::search_regex(
    ...
    while (std::regex_search(
        search_start, super_fast_search_index.cend(), match, regex
    )) {
        std::vector<fz_rect> match_rects;
        std::vector<fz_rect> compressed_match_rects;
