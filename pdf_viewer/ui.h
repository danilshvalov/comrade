...
class BaseSelectorWidget : public QWidget {
  ...
  protected:
    BaseSelectorWidget(QStandardItemModel* item_model, QWidget* parent)
        : QWidget(parent) {
    ...
    virtual bool on_text_change(const QString& text) { return false; }
  ...
  public:
        ...
        if (obj == line_edit) {
            ...
            if (event->type() == QEvent::KeyPress) {
                ...
                if ((key_event->key() == Qt::Key_PageDown)) {
                    QKeyEvent* new_key_event = new QKeyEvent(
                        key_event->type(), Qt::Key_PageDown,
                        key_event->modifiers()
                    );
                    QCoreApplication::postEvent(get_view(), new_key_event);
                    return true;
        ...
        if (selected_index_list.size() > 0) {
            ...
            if (proxy_model->hasIndex(
                ...
                QModelIndex source_index =
                    proxy_model->mapToSource(selected_index);
    ...
    virtual void on_config_file_changed() {
        ...
        // get_view()->setStyleSheet(get_view_stylesheet_type_name() +
        // "::item::selected{background-color: white; color: black;}");
...
template <typename T>
class FilteredTreeSelect
    : public BaseSelectorWidget<T, QTreeView, MySortFilterProxyModel> {
  private:
  ...
  public:
    ...
    FilteredTreeSelect(
        ...
        : BaseSelectorWidget<T, QTreeView, MySortFilterProxyModel>(
              item_model, parent
          ),
          on_done(on_done) {
...
template <typename T>
class FilteredSelectTableWindowClass
    : public BaseSelectorWidget<T, QTableView, MySortFilterProxyModel> {
  private:
  ...
  public:
    ...
    FilteredSelectTableWindowClass(
        ...
        : BaseSelectorWidget<T, QTableView, MySortFilterProxyModel>(
              nullptr, parent
          ),
          values(values),
          on_done(on_done),
...
template <typename T>
class FilteredSelectWindowClass
    : public BaseSelectorWidget<T, QListView, MySortFilterProxyModel> {
  private:
    QStringListModel* string_list_model = nullptr;
    std::vector<T> values;
    std::function<void(T*)> on_done = nullptr;
  ...
  public:
    ...
    FilteredSelectWindowClass(
        ...
        : BaseSelectorWidget<T, QListView, MySortFilterProxyModel>(
              nullptr, parent
          ),
          values(values),
          on_done(on_done),
...
class CommandSelector : public BaseSelectorWidget<
  ...
  private:
    ...
    QStandardItemModel*
    get_standard_item_model(std::vector<std::string> command_names) {
  ...
  public:
    ...
    CommandSelector(
        ...
        : BaseSelectorWidget<std::string, QTableView, MySortFilterProxyModel>(
              nullptr, parent
          ),
          key_map(key_map),
          on_done(on_done) {
    ...
    virtual bool on_text_change(const QString& text) {
        ...
        for (int i = 0; i < string_elements.size(); i++) {
            std::string encoded =
                utf8_encode(string_elements.at(i).toStdWString());
            int score = 0;
            if (FUZZY_SEARCHING) {
        ...
        for (int i = 0; i < string_elements.size(); i++) {
            if (string_elements.at(i).startsWith(text)) {
                matching_element_names.push_back(
                    string_elements.at(i).toStdString()
                );
            }
        }
        ...
        for (auto [command, score] : match_score_pairs) {
            if (score > 60 &&
                (!QString::fromStdString(command).startsWith(text))) {
        ...
        dynamic_cast<QTableView*>(get_view())
            ->setModel(new_standard_item_model);
...
class FileSelector : public BaseSelectorWidget<
  ...
  public:
    ...
    FileSelector(
        ...
        : BaseSelectorWidget<std::wstring, QListView, MySortFilterProxyModel>(
              nullptr, parent
          ),
          on_done(on_done) {
    ...
    QStringList get_dir_contents(QString root, QString prefix) {
        ...
        if (res.size() == 0) {
            ...
            for (auto file : all_directory_files) {
                ...
                int score = 0;
                if (FUZZY_SEARCHING) {
                    score = static_cast<int>(rapidfuzz::fuzz::partial_ratio(
                        encoded_prefix, encoded_file
                    ));
                } else {
                    fts::fuzzy_match(
