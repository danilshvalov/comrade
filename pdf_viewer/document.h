...
class Document {
  ...
  private:
    ...
    // The reason that we use a hashmap for reference_indices and a vector for
    // figures is that the reference we are looking for is usually the last
    ...
    // we do some of the document processing in a background thread (for example
    // indexing all the figures/indices and computing page heights. we use this
    // pointer to notify the main thread when processing is complete.
  ...
  public:
    ...
    // void add_highlight_annotation(const Highlight& highlight, const
    ...
    fz_stext_page* get_stext_with_page_number(
        fz_context* ctx, int page_number, fz_document* doc = nullptr
    );
    ...
    int find_closest_bookmark_index(
        const std::vector<BookMark>& sorted_bookmarks, float to_offset_y
    ) const;
    int find_closest_highlight_index(
    ...
    bool open(
        bool* invalid_flag,
        bool force_load_dimensions = false,
        std::string password = "",
        bool temp = false
    );
    ...
    // float get_page_width_smart(int page_index, float* left_ratio, float*
    // right_ratio, int* normal_page_width);
    float get_page_size_smart(
        bool width,
        int page_index,
        float* left_ratio,
        float* right_ratio,
    ...
    void get_text_selection(
        AbsoluteDocumentPos selection_begin,
        AbsoluteDocumentPos selection_end,
        bool is_word_selection, // when in word select mode, we select entire
    ...
    int add_stext_page_to_created_toc(
    ...
    // void get_ith_next_line_from_absolute_y(float absolute_y, int i, bool
    // cont, float* out_begin, float* out_end);
    fz_rect get_ith_next_line_from_absolute_y(
        int page,
        int line_index,
        int i,
        bool cont,
