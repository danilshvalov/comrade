...
class MainWidget : public QWidget, ConfigFileChangeListener {
  ...
  public:
    ...
    // current widget responsible for selecting an option (for example toc or
    // bookmarks)
    ...
    // is the user currently selecing text? (happens when we left click and move
    // the cursor)
    ...
    std::optional<PdfViewOpenGLWidget::OverviewMoveData> overview_move_data =
        {};
    std::optional<PdfViewOpenGLWidget::OverviewResizeData>
    ...
    bool find_location_of_text_under_pointer(
        WindowPos pos,
        int* out_page,
        float* out_offset,
    ...
    void get_window_params_for_one_window_mode(
        int* main_window_size, int* main_window_move
    );
    void get_window_params_for_two_window_mode(
